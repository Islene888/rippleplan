...
- Exercised both partner integration paths in production; the v7 capture shows a validated live run while the product preserves honest `partial` and `reference` fallback states.
...
## Current local video candidate (v7, 2:30.9; not yet uploaded)
...
The local candidate is `../RipplePlan_Demo_v7.mp4` (1280×720, H.264/AAC). It supersedes v6. No public video URL exists yet, so this must not be described as uploaded or submitted.
...
- [x] Local v7 video candidate: 2:30.9, 1280×720, H.264/AAC
